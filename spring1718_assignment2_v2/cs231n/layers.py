...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        out_norm=(x-sample_mean)/np.sqrt(sample_var+eps)
        out=out_norm*gamma+beta
        running_mean=momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        cache =(x,gamma,sample_mean,sample_var,eps,out_norm)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    #pass
    x,gamma,sample_mean,sample_var,eps,out_norm=cache 
    N = x.shape[0]
    dx_hat = dout * gamma                   
    ...
    dgamma=np.sum(out_norm*dout,0)
    dbeta=np.sum(np.ones_like(out_norm)*dout,0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    ## 卷积开始 
    for n in range(N): 
        for f in range(F): ## 临时分配(new_H, new_W)大小的全偏移项卷积矩阵，（即提前加上偏移项b[f]） 
            conv_newH_newW = np.ones([new_H, new_W])*b[f] 
            for c in range(C): ## 填充原始矩阵，填充大小为pad，填充值为0 
